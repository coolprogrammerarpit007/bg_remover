...
from utils import remove_bg_bytes  # ⚠️ This may return dict or bytes
...
app = FastAPI(title="Background Remover API", version="2.1")
...
def error_response(message, diagnostic=None):
    return {
        "status": False,
        "message": message,
...
async def remove_background(data: ImageBase64, db: Session = Depends(get_db)):
    try:
        ...
        # Step 4: Background Removal (thread-safe)
        try:
            result = await run_in_threadpool(remove_bg_bytes, image_bytes)
        except TimeoutError:
        ...
        # Step 5: Interpret result correctly
        if isinstance(result, dict):
            # Diagnostic response returned instead of bytes
            if not result.get("status"):
                diag = result.get("diagnostic", {})
                logger.error(f"Background removal diagnostic: {diag}")
                return error_response(result.get("message", "Processing failed"), diag)

            # Success case: extract bytes
            processed_bytes = result.get("data", None)
            if not processed_bytes:
                return error_response("Processing succeeded but no image data returned")

        elif isinstance(result, (bytes, bytearray)):
            processed_bytes = result
        else:
            raise TypeError(f"Unexpected result type from remove_bg_bytes: {type(result)}")
        ...
        logger.error(f"Type Error: {type_err}")
        return error_response(
            f"Server Error: {type_err}",
            diagnostic={"hint": "Expected bytes, but received structured response. Check remove_bg_bytes() return type."}
        )
